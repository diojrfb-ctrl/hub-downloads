...
class ScraperManager {
  ...
  async run(url) {
    try {
      ...
      if (error2 instanceof Error) message = error2.message;
      return {
        success: false,
        error: message
      };
...
function createWindow() {
  ...
  ipcMain.handle("scrape-media", async (_event, url) => {
...
main$1.autoUpdater.on("update-available", () => {
  win == null ? void 0 : win.webContents.send("update-message", "Atualização disponível...");
});
main$1.autoUpdater.on("update-downloaded", () => {
  win == null ? void 0 : win.webContents.send("update-message", "Atualização baixada. Reinicie para aplicar.");
});
